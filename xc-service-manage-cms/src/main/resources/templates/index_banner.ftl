    ...
    <body>
            ...
            <div class="banner-item">
                <#if model??>
                    <#list model as item>
                        <#if item_index == 0>
                            <div class="item"
                                 style="background-image: url(${item.value});z-index: 1;'opacity': '1'"></div>
                        </#if>
                        <#if item_index != 0>
                            <div class="item"
                                 style="background-image: url(${item.value});"></div>
                        </#if>
                    </#list>
                </#if>
            </div>
        ...
        <script type="text/javascript">
            ...
            for (i = 0; i < tg.length; i++) {
                if (i === 0)
                    $('.indicators').append('<span class="active"></span>');
                else
                    $('.indicators').append('<span></span>');
            }

            function roll() {
                tg.eq(num).stop(true, true).animate({
                    'opacity': '1',
                    'z-index': num
